...
function nativeDurationSegment(durationMs: number | null): Pick<
  EventSegment,
  "operationDurationMs" | "operationDurationSource" | "operationDurationConfidence"
> {
...
function extractOpenCodeToolDurationMs(partData: Record<string, unknown>): number | null {
  ...
  return extractOpenCodeDurationFromTime(asRecord(state?.time));
}
