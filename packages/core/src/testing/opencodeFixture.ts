...
export type OpenCodeFixtureInput = {
  rootDir: string;
  projectId?: string;
...
export function createOpenCodeFixtureDatabase(input: OpenCodeFixtureInput): {
  dbPath: string;
} {
  ...
  const projectTime = input.sessions[0]?.timeCreated ?? Date.now();
  db.prepare(
    `INSERT INTO project (
    ...
    projectId,
    input.worktree ?? input.sessions[0]?.directory ?? "/workspace",
    input.projectName ?? null,
    projectTime,
    input.sessions.at(-1)?.timeUpdated ?? input.sessions.at(-1)?.timeCreated ?? projectTime,
  );
    ...
    for (const message of session.messages) {
      ...
      for (const [partIndex, part] of (message.parts ?? []).entries()) {
        insertPart.run(
          ...
          message.id,
          session.id,
          message.timeCreated + partIndex,
          message.timeUpdated ?? message.timeCreated + partIndex,
          JSON.stringify(part),
        );
      }
    }
